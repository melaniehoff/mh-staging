		...
		<details data-path="<?php echo $base . $_key ?>" <?php echo $depth == 0 ? 'open' : '' ?>>
			<summary>
				...
				<div class="actions">
					...
					<button title="New Page" data-path="<?php echo $base . $_key ?>" onclick="newFile(this.dataset.path).then((el) => { this.parentElement.parentElement.parentElement.querySelector('ol').appendChild(el); });">+ 📄</button>
					...
					<button title="Upload" data-path="<?php echo $base . $_key ?>" onclick="uploadContextPath = this.dataset.path; insertUpload = false; document.getElementById('upload').click();">Upload</button>
					<?php if ($depth > 0) : ?>
					...
					<?php endif; ?>
                    <?php if ($depth == 0) : ?>
                        <button title="Rebuild" data-path="<?php echo $base . $_key ?>" onclick="alert('Requesting to rebuild all pages.'); rebuildAll(this.dataset.path);">Rebuild</button>
                    <?php endif; ?>
				</div>
			</summary>
...
<head>
	...
	<script>
        ...
        async function rebuildAll(path) {
            const res = await fetch('/cms/build.php' + path, {
                method: 'POST',
            });
            if (res.status != 200) {
                const body = await res.text();
	...
	<div class="container">
		<div class="panel" id="panel_editor">
			...
			<div class="controls">
				...
				<button id="toggle-cheatsheet" data-path="/assets/" onclick="uploadContextPath = this.dataset.path; insertUpload = true; document.getElementById('upload').click();">Upload</button>
